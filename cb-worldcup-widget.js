(()=>{const API="https://script.google.com/macros/s/AKfycbw438g3px7gBKhfn0agVBmG5nDPlltf7vRfsdA5gh6o0mkMaF1Fdjgz43-A4SRexXSF/exec",SBO="https://click-lynk.com/SBO_CBWL",BLOCK="/games/sport/sbo";
const flag={"Mexico":"mx","South Africa":"za","Korea Republic":"kr","Czechia":"cz","Canada":"ca","Bosnia and Herzegovina":"ba","United States":"us","USA":"us","Paraguay":"py","Qatar":"qa","Switzerland":"ch","Brazil":"br","Morocco":"ma","Haiti":"ht","Scotland":"gb-sct","Australia":"au","Turkiye":"tr","Turkey":"tr","Germany":"de","Curacao":"cw","Netherlands":"nl","Japan":"jp","Ivory Coast":"ci","Ecuador":"ec","Sweden":"se","Tunisia":"tn","Spain":"es","Cape Verde":"cv","Belgium":"be","Egypt":"eg","Saudi Arabia":"sa","Uruguay":"uy","Iran":"ir","New Zealand":"nz","France":"fr","Senegal":"sn","Iraq":"iq","Norway":"no","Argentina":"ar","Algeria":"dz","Austria":"at","Jordan":"jo","Portugal":"pt","DR Congo":"cd","Congo DR":"cd","England":"gb-eng","Croatia":"hr","Ghana":"gh","Panama":"pa","Uzbekistan":"uz","Colombia":"co"};
const isBlock=()=>location.pathname.toLowerCase().includes(BLOCK),remove=()=>{let w=document.getElementById("cb-worldcup-widget");if(w)w.remove()};
if(isBlock()){remove();return}
const esc=t=>String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");
const get=(m,a,b,c)=>m[a]||m[b]||m[c]||"",status=m=>String(get(m,"status","Status","")).toUpperCase();
const short=n=>({"Korea Republic":"South Korea","Bosnia and Herzegovina":"Bosnia & Her...","United States":"USA"}[n]||n);
const flagImg=n=>{let c=flag[String(n||"").trim()];return c?`<img class="cbwc-flag-img" src="https://flagcdn.com/w80/${c}.png" alt="${esc(n)}" loading="eager">`:`<span class="cbwc-ball">⚽</span>`};
const defOdd=(i,s)=>{let h=[.82,.8,.78,-.95,.65,-.86,.85,.72],a=[-.92,-.9,-.88,.85,-.75,.76,-.95,-.82];return(s==="h"?h:a)[i%8]};
const score=m=>{let a=get(m,"scoreA","Score A","score_a"),b=get(m,"scoreB","Score B","score_b"),s=status(m);if((s==="FINISHED"||s==="SELESAI")&&a!==""&&b!=="")return`<b>${esc(a)} - ${esc(b)}</b><br><span class="cbwc-ft">FT</span>`;if(s==="LIVE"&&a!==""&&b!=="")return`<b>${esc(a)} - ${esc(b)}</b><br><span class="cbwc-live-red">LIVE</span>`;return`${esc(get(m,"jam","Jam",""))}<br>WIB`};

function build(data){
let wrap=document.createElement("section");wrap.id="cb-worldcup-widget";
...
${(data||[]).slice(0,30).map((m,i)=>{let a=String(get(m,"teamA","Team A","team_a")),b=String(get(m,"teamB","Team B","team_b")),g=get(m,"group","Group / Stage","stage")||"WORLD CUP 2026",ho=get(m,"homeOdds","Home Odds","home_odds")||defOdd(i,"h"),ao=get(m,"awayOdds","Away Odds","away_odds")||defOdd(i,"a"),done=status(m)==="FINISHED"||status(m)==="SELESAI";return`<div class="cbwc-card"><div class="cbwc-league">🏆 ${esc(g)}</div><div class="cbwc-match"><div class="cbwc-team">${flagImg(a)}<span class="cbwc-name">${esc(short(a))}</span></div><div class="cbwc-mid"><div class="cbwc-date">${esc(get(m,"tanggal","Tanggal","date"))}</div><div class="cbwc-time">${score(m)}</div><div class="cbwc-live">⚽</div></div><div class="cbwc-team">${flagImg(b)}<span class="cbwc-name">${esc(short(b))}</span></div></div><div class="cbwc-market">${done?"MATCH SELESAI":"FT Handicap"}</div><div class="cbwc-odds"><a class="cbwc-odd" href="${SBO}" target="_blank" rel="noopener"><div class="cbwc-odd-label">Home 0</div><div class="cbwc-odd-num">${esc(ho)}</div></a><a class="cbwc-odd" href="${SBO}" target="_blank" rel="noopener"><div class="cbwc-odd-label">Away 0</div><div class="cbwc-odd-num">${esc(ao)}</div></a></div></div>`}).join("")}
...
let top=all.find(el=>{let t=(el.innerText||"").trim(),r=el.getBoundingClientRect();return t.includes("Top Games")&&r.width>200&&r.height>=20&&r.height<=220});
return top||document.querySelector('[class*="provider"],[class*="Provider"],[class*="game-provider"],[class*="GameProvider"]');
}
function insert(w){
if(isBlock()){remove();return 1}
if(document.getElementById("cb-worldcup-widget"))return 1;
let t=target();
if(t&&t.parentNode){t.parentNode.insertBefore(w,t);return 1}
let f=document.querySelector("main")||document.querySelector("#page-wrap")||document.querySelector("#root")||document.querySelector("#app")||document.body;
f.insertBefore(w,f.firstChild);return 1;
}
function nav(w){
...
let tries=0,timer=setInterval(()=>{tries++;if(isBlock()){remove();clearInterval(timer);return}if(document.getElementById("cb-worldcup-widget")){clearInterval(timer);return}let w=build(data);if(insert(w)){nav(w);clearInterval(timer)}if(tries>=60)clearInterval(timer)},400);
}
function boot(){
fetch(API+"?t="+Date.now()).then(r=>r.json()).then(r=>run(r.matches||r.data||[])).catch(err=>console.log("CB World Cup Widget Error:",err));
}
let last=location.href;
setInterval(()=>{if(location.href!==last){last=location.href;isBlock()?remove():boot()}},700);
boot();
})();
