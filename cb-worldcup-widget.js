...
function e(t){return String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}
function sn(n){return({"Korea Republic":"South Korea","Bosnia and Herzegovina":"Bosnia & Her...","United States":"USA"}[n]||n)}
function fl(n){let c=FC[String(n||"").trim()];return c?`<img class="cbwc-flag-img" src="https://flagcdn.com/w80/${c}.png" alt="${e(n)}" loading="eager">`:`<span class="cbwc-ball">⚽</span>`}
function od(i,s){let h=[.82,.8,.78,-.95,.65,-.86,.85,.72],a=[-.92,-.9,-.88,.85,-.75,.76,-.95,-.82];return(s==="h"?h:a)[i%8]}
function val(m,k1,k2,k3){return m[k1]||m[k2]||m[k3]||""}
function st(m){return String(val(m,"status","Status","")).toUpperCase()}
