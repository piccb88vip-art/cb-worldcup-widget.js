...
function put(){
 ...
 let t=target();if(!t||!t.parentNode)return;
 let w=make(DATA);t.parentNode.insertBefore(w,t);
...
function load(){
 fetch(API+"?t="+Date.now()).then(r=>r.json()).then(r=>{DATA=r.matches||r.data||[];READY=1;rm();put()}).catch(e=>console.log("CB World Cup Widget Error:",e));
}
setInterval(()=>{if(location.href!==LAST){LAST=location.href;rm();setTimeout(put,700)}if(bad())rm();else put()},900);
load();
})();
