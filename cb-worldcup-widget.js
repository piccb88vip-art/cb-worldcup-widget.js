(()=>{const API="https://script.google.com/macros/s/AKfycbxMoye9PO1d3bqIsDywSmStR9ynRpb8fxgcjqPqP2saZ98UZ_ektp5Dr0ldWzsNfb4x/exec",SBO="https://click-lynk.com/SBO_CBWL",BAD=["/games/","/sport/","/casino","/slot","/fishing","/togel","/promosi","/promotion","/bonus","/crash"],F={"Mexico":"mx","South Africa":"za","Korea Republic":"kr","Czechia":"cz","Canada":"ca","Bosnia and Herzegovina":"ba","United States":"us","USA":"us","Paraguay":"py","Qatar":"qa","Switzerland":"ch","Brazil":"br","Morocco":"ma","Haiti":"ht","Scotland":"gb-sct","Australia":"au","Turkiye":"tr","Turkey":"tr","Germany":"de","Curacao":"cw","Netherlands":"nl","Japan":"jp","Ivory Coast":"ci","Ecuador":"ec","Sweden":"se","Tunisia":"tn","Spain":"es","Cape Verde":"cv","Belgium":"be","Egypt":"eg","Saudi Arabia":"sa","Uruguay":"uy","Iran":"ir","New Zealand":"nz","France":"fr","Senegal":"sn","Iraq":"iq","Norway":"no","Argentina":"ar","Algeria":"dz","Austria":"at","Jordan":"jo","Portugal":"pt","DR Congo":"cd","Congo DR":"cd","England":"gb-eng","Croatia":"hr","Ghana":"gh","Panama":"pa","Uzbekistan":"uz","Colombia":"co"};let DATA=[],READY=0,LAST=location.href;
const bad=()=>BAD.some(x=>location.pathname.toLowerCase().includes(x)),rm=()=>document.querySelectorAll("#cb-worldcup-widget").forEach(x=>x.remove()),E=t=>String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),G=(m,a,b,c)=>m[a]||m[b]||m[c]||"",ST=m=>String(G(m,"status","Status","")).toUpperCase(),SN=n=>({"Korea Republic":"South Korea","Bosnia and Herzegovina":"Bosnia & Her...","United States":"USA"}[n]||n),FG=n=>{let c=F[String(n||"").trim()];return c?`<img class="cbwc-flag-img" src="https://flagcdn.com/w80/${c}.png" alt="${E(n)}">`:`<span class="cbwc-ball">⚽</span>`},OD=(i,s)=>{let h=[.82,.8,.78,-.95,.65,-.86,.85,.72],a=[-.92,-.9,-.88,.85,-.75,.76,-.95,-.82];return(s=="h"?h:a)[i%8]},SC=m=>{let a=G(m,"scoreA","Score A","score_a"),b=G(m,"scoreB","Score B","score_b"),s=ST(m);return(s=="FINISHED"||s=="SELESAI")&&a!==""&&b!==""?`<b>${E(a)} - ${E(b)}</b><br><span class="cbwc-ft">FT</span>`:s=="LIVE"&&a!==""&&b!==""?`<b>${E(a)} - ${E(b)}</b><br><span class="cbwc-live-red">LIVE</span>`:`${E(G(m,"jam","Jam",""))}<br>WIB`};
...
function target(){
 ...
 let top=all.filter(el=>{
  let t=(el.innerText||"").trim(),r=el.getBoundingClientRect();
  return t.includes("Top Games")&&r.width>80&&r.height>=20&&r.height<=190;
 }).sort((a,b)=>b.getBoundingClientRect().width-a.getBoundingClientRect().width)[0];

 if(top){
  let n=top,best=top;
  for(let i=0;i<12&&n.parentElement;i++,n=n.parentElement){
   let p=n.parentElement,r=p.getBoundingClientRect(),cs=getComputedStyle(p);
   if(r.width>=window.innerWidth*.75&&r.height>=35&&r.height<=260)best=p;
   if(cs.display==="grid"&&r.height>260)break;
  }
  return best;
 }

 let keys=["pragmatic","pgsoft","pocket","joker","jili","microgaming","nolimit","cq9","habanero","kingmaker"];
 let img=[...document.images].find(i=>{
  let x=((i.src||"")+" "+(i.alt||"")).toLowerCase();
 ...
 for(let i=0;i<12&&p;i++,p=p.parentElement){
  let r=p.getBoundingClientRect(),cs=getComputedStyle(p);
  if(r.width>=window.innerWidth*.75&&r.height>=35&&r.height<=260)best=p;
  if(cs.display==="grid"&&r.height>260)break;
 }
