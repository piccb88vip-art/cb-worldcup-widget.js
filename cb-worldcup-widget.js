(()=>{const API="https://script.google.com/macros/s/AKfycbw438g3px7gBKhfn0agVBmG5nDPlltf7vRfsdA5gh6o0mkMaF1Fdjgz43-A4SRexXSF/exec",SBO="https://click-lynk.com/SBO_CBWL";
const BAD=["/games/","/sport/","/casino","/slot","/fishing","/togel","/promotion","/promosi","/bonus","/crash"];
const F={"Mexico":"mx","South Africa":"za","Korea Republic":"kr","Czechia":"cz","Canada":"ca","Bosnia and Herzegovina":"ba","United States":"us","USA":"us","Paraguay":"py","Qatar":"qa","Switzerland":"ch","Brazil":"br","Morocco":"ma","Haiti":"ht","Scotland":"gb-sct","Australia":"au","Turkiye":"tr","Turkey":"tr","Germany":"de","Curacao":"cw","Netherlands":"nl","Japan":"jp","Ivory Coast":"ci","Ecuador":"ec","Sweden":"se","Tunisia":"tn","Spain":"es","Cape Verde":"cv","Belgium":"be","Egypt":"eg","Saudi Arabia":"sa","Uruguay":"uy","Iran":"ir","New Zealand":"nz","France":"fr","Senegal":"sn","Iraq":"iq","Norway":"no","Argentina":"ar","Algeria":"dz","Austria":"at","Jordan":"jo","Portugal":"pt","DR Congo":"cd","Congo DR":"cd","England":"gb-eng","Croatia":"hr","Ghana":"gh","Panama":"pa","Uzbekistan":"uz","Colombia":"co"};
let DATA=[],LAST=location.href;

const bad=()=>BAD.some(x=>location.pathname.toLowerCase().includes(x));
const rm=()=>{let w=document.getElementById("cb-worldcup-widget");if(w)w.remove()};
const esc=t=>String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");
const get=(m,a,b,c)=>m[a]||m[b]||m[c]||"";
const stat=m=>String(get(m,"status","Status","")).toUpperCase();
const sn=n=>({"Korea Republic":"South Korea","Bosnia and Herzegovina":"Bosnia & Her...","United States":"USA"}[n]||n);
const flag=n=>{let c=F[String(n||"").trim()];return c?`<img class="cbwc-flag-img" src="https://flagcdn.com/w80/${c}.png" alt="${esc(n)}" loading="eager">`:`<span class="cbwc-ball">⚽</span>`};
...
function target(){
  ...
  let top=all.find(el=>{
    ...
    return t.includes("Top Games")&&r.width>200&&r.height>=20&&r.height<=220;
  });
  if(top)return top;

  let imgs=[...document.images].filter(img=>{
    let x=((img.src||"")+" "+(img.alt||"")).toLowerCase();
    return x.includes("pragmatic")||x.includes("pgsoft")||x.includes("joker")||x.includes("jili");
  });
  ...
  for(let img of imgs){
    ...
    for(let i=0;i<8&&p;i++,p=p.parentElement){
      ...
      if(r.width>260&&r.height>=35&&r.height<=170)return p;
    }
  }
  return null;
}
...
</style><div class="cbwc-top"><div class="cbwc-icon">🏆</div><div class="cbwc-heading">Rekomendasi Pertandingan <span class="cbwc-brand">CLICKBET88</span></div></div><div class="cbwc-scroll-wrap"><button class="cbwc-nav cbwc-prev" type="button">‹</button><div class="cbwc-scroll">${(d||[]).slice(0,30).map((m,i)=>{let a=String(get(m,"teamA","Team A","team_a")),b=String(get(m,"teamB","Team B","team_b")),g=get(m,"group","Group / Stage","stage")||"WORLD CUP 2026",ho=get(m,"homeOdds","Home Odds","home_odds")||odd(i,"h"),ao=get(m,"awayOdds","Away Odds","away_odds")||odd(i,"a"),done=stat(m)==="FINISHED"||stat(m)==="SELESAI";return`<div class="cbwc-card"><div class="cbwc-league">🏆 ${esc(g)}</div><div class="cbwc-match"><div class="cbwc-team">${flag(a)}<span class="cbwc-name">${esc(sn(a))}</span></div><div class="cbwc-mid"><div class="cbwc-date">${esc(get(m,"tanggal","Tanggal","date"))}</div><div class="cbwc-time">${score(m)}</div><div class="cbwc-live">⚽</div></div><div class="cbwc-team">${flag(b)}<span class="cbwc-name">${esc(sn(b))}</span></div></div><div class="cbwc-market">${done?"MATCH SELESAI":"FT Handicap"}</div><div class="cbwc-odds"><a class="cbwc-odd" href="${SBO}" target="_blank" rel="noopener"><div class="cbwc-odd-label">Home 0</div><div class="cbwc-odd-num">${esc(ho)}</div></a><a class="cbwc-odd" href="${SBO}" target="_blank" rel="noopener"><div class="cbwc-odd-label">Away 0</div><div class="cbwc-odd-num">${esc(ao)}</div></a></div></div>`}).join("")}</div><button class="cbwc-nav cbwc-next" type="button">›</button></div>`;
return w;
}
...
function load(){
  ...
  .then(r=>r.json())
  .then(r=>{DATA=r.matches||r.data||[];put()})
  .catch(e=>console.log("CB World Cup Widget Error:",e));
}

setInterval(()=>{
  if(location.href!==LAST){LAST=location.href;rm();setTimeout(()=>{if(!bad())put()},800)}
  if(bad())rm();
